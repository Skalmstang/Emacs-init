...
(defun try-require (FEATURE)
  "Like require but does not signal an error when failing to load"
  (when (symbolp FEATURE)
    (let ((name (symbol-name FEATURE)))
      (if (require FEATURE name t)
	  t
	(message "%s failed loading." name)
	(setq munk-features-failed-loading
	      (append munk-features-failed-loading (list FEATURE)))))))

(defvar munk-features-failed-loading nil
  "A list the features that failed loading")

(defun startup-echo-area-message ()
  (concat
   (number-to-string (length munk-features-failed-loading))
   " features failed loading."))
